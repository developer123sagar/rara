...
const RestaurantSlider: React.FC<ImageSliderProps> = ({
  images,
  autoSlide = true,
  interval = 5000,
  slidesToShow = 1,
  ...
  return (
    <div className="relative overflow-hidden mt-20 r-2xl:mt-[-14px] group">
        ...
        <div className="flex gap-10">
          {Array.from({ length: slidesToShow }).map((_, index) => (
            <motion.img
              ...
              className={`w-[${width}%] mx-auto h-[70vh] object-cover transition-transform duration-500 ease-in-out`}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            />
