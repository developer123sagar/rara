...
import { fetchOrderPercentData } from "@/redux/foods/foodDetailSlice";
...
const options: ApexOptions = {
  ...
  colors: ["#e01f2d", "#27e018"],
  labels: ["Home delivery", "Pickup "],
  legend: {
    show: true,
    position: "bottom",
  },
...
const ChartThree: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(fetchOrderPercentData({ token }));
  }, [dispatch, token]);
  ...
  const restaurantNumberData: {
    ...
    (state: RootState) => state.foodDetails.orderPercentageData
  );

  const state = {
    series: [
      restaurantNumberData.pickupPercentage,
      restaurantNumberData.homeDeliveryPercentage,
    ],
  };
  ...
  return (
    <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pt-7.5 pb-5 shadow-default   sm:px-7.5 xl:col-span-5">
      <div className="mb-3 justify-between gap-4 sm:flex">
        <div>
          <h5 className="text-xl font-semibold text-black ">Order Analytics</h5>
        </div>
        <div>
        ...
        <div className="w-full px-8 sm:w-1/2">
          <div className="flex w-full items-center">
            ...
            <p className="flex w-full justify-between text-sm font-medium text-black ">
              <span> Delivery </span>
              <span> {state?.series[0]?.toString()}% </span>
            </p>
          </div>
        </div>
        <div className="w-full px-8 sm:w-1/2">
          <div className="flex w-full items-center">
            ...
            <p className="flex w-full justify-between text-sm font-medium text-black ">
              <span> Pickup </span>
              <span> {state?.series[1]?.toString()}% </span>
            </p>
          </div>
        </div>
      </div>
